...
public class Login {
	...
	/**
	 * 로그인 기능을 수행하는 메인 메소드 이 메소드는 사용자로부터 아이디와 비밀번호를 입력받아 로그인을 시도하고,
	 * 회원 정보를 검증하여 로그인 여부를 결정합니다.
	 */
	public static void login() {
		...
		if (Login.login != null) {
			...
			user = new User(login.getNo(), login.getName(), login.getId(), login.getPwd(), login.getTel(),
					login.getJumin(), login.getEmail(), login.getAddress(), login.getAccount(), login.getMoney(),
					login.getGrade());
