...
public class BuyList {
	...
	private void myBuyList(User user) {
		...
		System.out.println(
				"[번호]           [품명]             [카테고리]           [상품품질]   [가격]      [거래방법]      [지불방법]     [거래일자]      [판매자]      [구매자]");
		for (int i = 0; i < Data.soldOutArrayList.size(); i++) {
			if (user.getNumber().equals(Data.soldOutArrayList.get(i).getBuyNum())) {
				...
				System.out.printf("%5s %10s\t%8s\t  %12s\t   %8s  %10s %11s %16s", item.getNum(), item.getName(),
						Constant.Category(item.getCategory()), Constant.Condition(item.getStatus()), item.getPrice(),
						Constant.Method(item.getTransactionMethod()), Constant.Payment(item.getPaymentMethod()),
						item.getTransactionData());
	...
	public void buyTest(SellingStuff s) {
		...
		System.out.printf("%5s\t%-14s\t%s\t%9s\t%8s\t%-6s\t\t%-13s\t%-15s\t%-14s\t%3s\r\n", s.getNo(), s.getName(),
				Constant.Condition(s.getCondition()), s.getPrice(), s.getSellerNo(), Constant.Method(s.getMethod()),
